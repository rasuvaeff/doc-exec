...
- Сгенерированный скрипт и результаты, которые он сообщает, пишутся во
  временные файлы, созданные `tempnam()` (режим 0600, эксклюзивно), и
  исполняются массивной формой `proc_open` — шелл не участвует нигде — а оба
  файла удаляются даже при исключении.
